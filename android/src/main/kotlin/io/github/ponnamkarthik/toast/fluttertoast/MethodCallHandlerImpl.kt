    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "showToast" -> {
                ...
                if (bgcolor != null) {
                    val toastCustomLayoutId = context.resources.getIdentifier("toast_custom", "layout", context.packageName)
                    val layout = (context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater).inflate(toastCustomLayoutId, null)
                    val textId = context.resources.getIdentifier("text", "id", context.packageName)
                    val text = layout.findViewById<TextView>(textId)
                    text.text = mMessage

                    val cornerId = context.resources.getIdentifier("corner", "drawable", context.packageName)
                    val gradientDrawable: Drawable? = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        context.getDrawable(cornerId)!!
                    } else {
                        ContextCompat.getDrawable(context, cornerId)
                    }
